...
- **Image Asset Standard (Permanent Universal Rule)**: **Optimized WebP for raster website images by default; preserve native SVG for vector graphics**. Retain master/original files on disk as source archives/fallbacks. Avoid unnecessary re-encoding of already optimized assets to prevent generational loss. Thoroughly verify visual quality, sharpness, alpha transparency, and intrinsic dimensions (`naturalWidth`/`naturalHeight`) before replacing or swapping assets to prevent visual bugs and layout shifts (zero CLS). Never reference raw heavy PNG/JPEG files in production HTML/CSS when optimized WebP can be used.

- **Universal Senior Engineering Development Rules**: Installed globally at `~/.gemini/config/rules/universal-development-rules.md` (and mandated via `~/.gemini/config/AGENTS.md`) and at project level in `.agents/rules/universal_development_rules.md` (referenced in `AGENTS.md`). Governs all 23 standard engineering phases: project documentation initialization (`PRD.md`, `TechSpec.md`, `AppFlow.md`, `Design.md`, `Schema.md`, `ImplementationPlan.md`, `Tracker.md`, `Rules.md`, `LaunchChecklist.md`), impact tracing, server-side security enforcement, data integrity, truthful success reporting, deliberate failure handling, accessibility, performance budgeting, observable multi-workflow verification, and evidence-based release safety.
...
2. **Dedicated Embroidery Specialist Pages (5 Dedicated URLs & Reusable 11-Section Template)**:
   ...
   - **Page 2: Left Chest Digitizing (`/embroidery-digitizing/left-chest-digitizing/`)**:
      - Replicates the exact layout, spacing, cards, FAQ style, and design system of the Cap & Hat Digitizing page.
      - **Hero**: Flat rate $15, focused on polos, uniforms, work shirts, corporate apparel, small logos and lettering.
      - **Top Production Comparison (Section 2)**: Atkins Communications Group — Side-by-side CAD preview video (8s) and final stitch-out across Port Authority polos and Under Armour outerwear.
      - **Work Showcase (Section 6: Recent Left Chest Projects)**:
        1. **Case Study 1 (School Crest & Heavy-Duty Gear)**: Appoquinimink High School Jaguars — Left column with 8s CAD simulation video (`videos/appoquinimink-crest-digitized-preview.mp4`); Right column with CAD software preview and finished heavy poly-canvas backpack crest with personalized name `T. THOMAS` (`images/appoquinimink-jaguars-left-chest-crest-embroidery.webp`).
        2. **Case Study 2 (Commercial Uniforms & Multi-Polo Batch)**: Marlow Electric — Left column with 11s Tajima CAD software simulation video (`videos/marlow-electric-digitized-preview.mp4`); Right column with CAD micro-text sequence preview (`images/marlow-electric-digitized-software-preview.webp`) and finished 5-polo commercial batch across heather pique polos with 302-750-4948 tagline (`images/merlow-electric-polo-batch-embroidery.webp`).
      - All media strictly encoded in modern WebP and optimized MP4 format with zero CLS explicit dimensions and responsive Playwright verification.
...
- `/pricing.html` (Clean route `/pricing`): Dedicated flat-rate Pricing showcase in responsive layout, strictly adhering to Dezan's brand color scheme (Dezan Gold `#d4af35` / `#9a7810`, Dark Luxury `#201d12`, Card Dark `#16140c`, Warm Canvas `#f8f7f6`):
  - **SEO Metadata**: Title: `Embroidery Digitizing Pricing | Flat Rates from $15 | Dezan Digitizing`, Meta Description: `See Digitizing pricing for hats, left chest, jacket back, large design and pet portrait embroidery digitizing. Flat rates start at $15 for embroidery shops across the U.S.`, Canonical URL: `https://dezandigitizing.com/pricing`.
