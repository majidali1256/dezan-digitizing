...
- **Universal Senior Engineering Development Rules**: Installed globally at `~/.gemini/config/rules/universal-development-rules.md` (mandated via `~/.gemini/config/AGENTS.md`) and in workspace `.agents/rules/universal_development_rules.md` (referenced in `AGENTS.md`). Governs 10 core senior engineering and design principles: 1. Understand First, 2. Make Focused Complete Changes, 3. Respect Existing Design, 4. Protect Security and Data, 5. Handle Real-World Failure, 6. Verify Proportionally, 7. Keep Quality Practical, 8. Keep Documentation Useful, 9. Work Within Authorization, 10. Finish Honestly.
