...
(function () {
    ...
    function renderClientOrderCard(order, isCompleted) {
        ...
        const downloadUrl = (hasDeliverables && deliverables[0].url)
            ? deliverables[0].url
            : (order.deliverable_url || order.artwork_url || '#');
        ...
        return `
            <div id="client-card-${order.order_number}" class="client-order-card digitizer-bento-card p-4 sm:p-5 rounded-2xl bg-white dark:bg-card-dark ${cardThemeClass} shadow-xs hover:shadow-md transition-all flex flex-col justify-between group">
                ...
                <!-- Actions Toolbar -->
                ${isCompleted ? `
                    <!-- Completed Order Actions: Secondary Row (View Order + Request Revision) & Prominent Download Files Button -->
                    <div class="pt-2.5 border-t border-slate-100 dark:border-primary/10 mt-2 space-y-2">
                        <!-- Secondary Action Row -->
                        <div class="flex items-center justify-between gap-2">
                            <button type="button" onclick="window.clientWorkspace.openOrderDetailsModal('${order.order_number}')" class="px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-bold text-xs inline-flex items-center gap-1 cursor-pointer transition-colors whitespace-nowrap shrink-0" title="Open dedicated order detail screen">
                                <span>View Order</span>
                                <span class="material-symbols-outlined text-xs text-amber-600 dark:text-primary">arrow_forward</span>
                            </button>
                            <button type="button" onclick="window.clientWorkspace.openRevisionModal('${order.order_number}')" class="px-2.5 py-1.5 rounded-xl border border-purple-300 dark:border-purple-600/50 hover:border-purple-400 dark:hover:border-purple-400 text-purple-700 dark:text-purple-300 hover:bg-purple-50/80 dark:hover:bg-purple-950/40 bg-purple-50/40 dark:bg-purple-950/20 font-bold text-xs inline-flex items-center gap-1 cursor-pointer transition-all whitespace-nowrap shrink-0" title="Request revision for this order">
                                <span class="material-symbols-outlined text-xs text-purple-600 dark:text-purple-400">refresh</span>
                                <span>Request Revision</span>
                            </button>
                        </div>
                        <!-- Prominent Primary Download Files Button (70-85% card width, centered) -->
                        <div class="flex justify-center pt-0.5 w-full">
                            <a href="${downloadUrl}" download target="_blank" onclick="window.clientWorkspace.downloadDeliverablesForOrder('${order.order_number}', event)" class="w-[82%] py-2 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs inline-flex items-center justify-center gap-1.5 shadow-xs hover:shadow-md transition-all active:scale-[0.99] cursor-pointer" title="Download completed embroidery files">
                ...
                ` : `
                    <!-- Actions Toolbar (Active / Due / Quote Orders) -->
    ...
    function downloadDeliverablesForOrder(orderNumber, e) {
        const order = (state.orders || []).find(o => o.order_number === orderNumber || o.id === orderNumber);
        if (!order) return;
        const delivs = (Array.isArray(order.deliverables) && order.deliverables.length > 0)
            ? order.deliverables
        ...
        if (delivs.length > 1) {
            ...
            delivs.forEach((del, idx) => {
                setTimeout(() => {
                    ...
                    a.href = del.url;
                    a.download = del.name || `${order.order_number || 'deliverable'}_${idx + 1}`;
                    a.target = '_blank';
